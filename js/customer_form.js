...
// 配置
// =====================
const APP_ID     = 'cli_a9365221543a5ccc';
const APP_SECRET = 'o08J7194FRXuGPffPICd6cHb6gyKWjBY';
...
async function getAccessToken() {
  const data = await apiRequest('/auth/v3/app_access_token/internal', 'POST', {
    app_id: APP_ID, app_secret: APP_SECRET
  });
  if (data.code === 0) return data.app_access_token;
  throw new Error(`获取 App Token 失败: ${data.msg}`);
}
...
class CustomerForm {
  ...
  _getListPageUrl() {
    // 客户列表页路径（根据项目结构调整）
    return '../index.html?page=customer_list';
  }
